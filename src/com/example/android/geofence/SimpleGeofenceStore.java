...
public class SimpleGeofenceStore {
    ...
    public List<SimpleGeofence> getGeofences(){
    	...
    	nGeofences=getNGeofences();
    	if(nGeofences==GeofenceUtils.INVALID_INT_VALUE) {
    		Log.e("getGeofences", "nGeofences is invalid_int_value");
    		return null;
    	}
    	
    	List<SimpleGeofence> simpleGeofences=new LinkedList<SimpleGeofence>();
    	
    	for(int i=1;i <= nGeofences; i++){
    		
    		sGeofence=getGeofence(Integer.toString(i));
    		if(sGeofence!=null) simpleGeofences.add(sGeofence);
    		else{
    			Log.e("GetGeofences", "error while building geofence list");
    			return null;
    		}
    	}
